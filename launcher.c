// Source code to build `cli.exe`, copied from setuptools.
// To build simply run `gcc -DGUI=0 launcher.c -o cli.exe
// This `cli.exe` can be renamed to `foo.exe` and placed alongside
// `foo-script.py` to make it run `foo-script.py`.
// The `foo-script.py` must contain a shebang with the Python executable to use.
